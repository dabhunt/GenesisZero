...
public class DialogueManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        UnityEngine.Object[] objs = Resources.LoadAll("Assets/Resources/Dialogue/CharacterIcons");
        iconArray = new Sprite[objs.Length];   
        for (int i = 0; i < objs.Length; i++)
        {
            iconArray[i] = (Sprite)objs[i];
        }
    ...
    IEnumerator TypeSentence(string charIcon, string sentence, string dur)
    {
        ...
        charIcon.ToUpper();
        //Sprite spr = Resources.Load<Sprite>("Assets/Resources/Dialogue/CharacterIcons/" + charIcon);
        Sprite spr = null;
        //print("spr.name: " + spr.name);
        for (int i = 0; i < iconArray.Length; i++)
        {
            if (iconArray[i].name == charIcon)
            {
                spr = iconArray[i];
                print("name match!");
                break;
